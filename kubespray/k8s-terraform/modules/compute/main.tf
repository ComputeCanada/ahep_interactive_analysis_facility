...
resource "openstack_compute_instance_v2" "bastion" {
  ...
  provisioner "local-exec" {
    command = "sed s/USER/${var.ssh_user}/ ansible_bastion_template.txt | sed s/BASTION_ADDRESS/${var.bastion_fips[0]}/ > group_vars/no_floating.yml"
  }
}

resource "openstack_compute_instance_v2" "k8s_master" {
  name              = "${var.cluster_name}-k8s-master-x-${count.index + 1}"
  ...
  provisioner "local-exec" {
    command = "sed s/USER/${var.ssh_user}/ ansible_bastion_template.txt | sed s/BASTION_ADDRESS/${element(concat(var.bastion_fips, var.k8s_master_fips), 0)}/ > group_vars/no_floating.yml"
  }
}
...
resource "openstack_compute_instance_v2" "k8s_node" {
  name              = "${var.cluster_name}-k8s-node-x-${count.index + 1}"
  ...
  provisioner "local-exec" {
    command = "sed s/USER/${var.ssh_user}/ ansible_bastion_template.txt | sed s/BASTION_ADDRESS/${element(concat(var.bastion_fips, var.k8s_node_fips), 0)}/ > group_vars/no_floating.yml"
  }
}
...
resource "openstack_compute_instance_v2" "k8s_nodes" {
  ...
  provisioner "local-exec" {
    command = "%{if each.value.floating_ip}sed s/USER/${var.ssh_user}/ ansible_bastion_template.txt | sed s/BASTION_ADDRESS/${element(concat(var.bastion_fips, [for key, value in var.k8s_nodes_fips : value.address]), 0)}/ > group_vars/no_floating.yml%{else}true%{endif}"
  }
}
